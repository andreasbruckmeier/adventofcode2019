...
# this works with sample data and is for sure no general solution
you_path = [[y.name for y in x] for x in paths if x[-1].name == 'YOU'][0]
santa_path = [[y.name for y in x] for x in paths if x[-1].name == 'SAN'][0] 
 
# go from you until we meet an intersection with santas path
count = 0
for i in range(len(you_path) - 2, -1, -1):
  count += 1
  if you_path[i] in santa_path:
    count += len(santa_path) - 3 - santa_path.index(you_path[i])
    break;
